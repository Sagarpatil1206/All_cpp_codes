#include<iostream>
using namespace std;
int getBit(int n,int pos){//finding bit of desired position
    if(( n & (1<<pos))!=0){return 1;}
    else{return 0;}}

int setBit(int n,int pos){//setting bit (1) at desired position
    return (n | (1<<pos));}

int clearBit(int n,int pos){//clearing a bit of desired position
        int mask= ~(1<<pos);
        return (n & mask);
}
int updateBit(int n,int pos,int value){//updating desired bit at desired index
    int mask=~(1<<pos);

...
T ExpressionConversion<T>::infixToPostfix(T s)// for converting infix to postfix
{
    ...
    cout << "Symbol\t\t"
         ...
         << "Expression\t\t" << endl;
    int i;
    for (i = 0; i < s.length(); i++)//traversing over a infix expression
    {
        if ((s[i] >= 'a' && s[i] <= 'z') || (s[i] >= 'A' && s[i] <= 'Z'))
        {
            ans += s[i];//adding into th ans string
            //cout<<"ans string is "<<ans<<endl;
        }
        else if (s[i] == '(')//pushing ( into the stack
        {
            st.push(s[i]);
        }
        else if (s[i] == ')')//if ) strikes we make whole stack empty until we got ( bracket
        {
        ...
        {//checking associativity and precedance
